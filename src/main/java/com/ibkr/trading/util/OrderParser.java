package com.ibkr.trading.util;

import java.time.LocalDate;
import java.time.format.DateTimeFormatter;
import java.time.format.DateTimeParseException;
import java.util.Locale;
import java.util.regex.Matcher;
import java.util.regex.Pattern;

/**
 * Utility for parsing natural language order strings into structured order data.
 * 
 * Supported format: "DD MMM YY BUY/SELL SYMBOL CALL/PUT STRIKE"
 * Example: "15 Mar 24 SELL SPY CALL 600"
 * 
 * @author IBK Trading System
 * @version 2.0
 */
public class OrderParser {
    
    /**
     * Represents a successfully parsed order.
     */
    public static class ParsedOrder {
        public final LocalDate expiryDate;
        public final String action;
        public final String symbol;
        public final String optionType;
        public final double strikePrice;
        public final boolean isValid;
        public final String errorMessage;
        
        private ParsedOrder(LocalDate expiryDate, String action, String symbol, 
                           String optionType, double strikePrice, boolean isValid, String errorMessage) {
            this.expiryDate = expiryDate;
            this.action = action;
            this.symbol = symbol;
            this.optionType = optionType;
            this.strikePrice = strikePrice;
            this.isValid = isValid;
            this.errorMessage = errorMessage;
        }
        
        public static ParsedOrder valid(LocalDate expiryDate, String action, String symbol, 
                                       String optionType, double strikePrice) {
            return new ParsedOrder(expiryDate, action, symbol, optionType, strikePrice, true, null);
        }
        
        public static ParsedOrder invalid(String errorMessage) {
            return new ParsedOrder(null, null, null, null, 0.0, false, errorMessage);
        }
        
        /**
         * Formats expiry date in IB API format (yyyyMMdd).
         */
        public String getFormattedExpiry() {
            if (!isValid || expiryDate == null) return "";
            return expiryDate.format(DateTimeFormatter.ofPattern("yyyyMMdd"));
        }
        
        /**
         * Gets option right for IB API ("C" or "P").
         */
        public String getOptionRight() {
            if (!isValid || optionType == null) return "";
            return optionType.equals("CALL") ? "C" : "P";
        }
        
        @Override
        public String toString() {
            if (!isValid) return "Invalid: " + errorMessage;
            return String.format("%s %s %s $%.2f (Expiry: %s)", 
                action, symbol, optionType, strikePrice, expiryDate);
        }
    }
    
    private static final Pattern ORDER_PATTERN = Pattern.compile(
        "(\\d{1,2})\\s+" +                      // Day (1-31)
        "(jan|feb|mar|apr|may|jun|jul|aug|sep|oct|nov|dec)\\s+" +  // Month
        "(\\d{2,4})\\s+" +                      // Year (2-4 digits)
        "(buy|sell)\\s+" +                      // Action
        "(\\w+)\\s+" +                          // Symbol
        "(call|put)\\s+" +                      // Option type
        "(\\d+(?:\\.\\d+)?)",                   // Strike price
        Pattern.CASE_INSENSITIVE
    );
    
    /**
     * Parses a natural language order string into a ParsedOrder object.
     * 
     * @param input the order string to parse
     * @return ParsedOrder with validation status and parsed values
     */
    public static ParsedOrder parseOrder(String input) {
        if (input == null || input.trim().isEmpty()) {
            return ParsedOrder.invalid("Empty input");
        }
        
        input = input.trim().toLowerCase();
        
        Matcher matcher = ORDER_PATTERN.matcher(input);
        
        if (!matcher.find()) {
            return ParsedOrder.invalid(
                "Invalid format. Expected: 'DD MMM YY BUY/SELL SYMBOL CALL/PUT STRIKE'\n" +
                "Example: '15 Mar 24 SELL SPY CALL 600'"
            );
        }
        
        try {
            int day = Integer.parseInt(matcher.group(1));
            String monthStr = matcher.group(2);
            String yearStr = matcher.group(3);
            String action = matcher.group(4).toUpperCase();
            String symbol = matcher.group(5).toUpperCase();
            String optionType = matcher.group(6).toUpperCase();
            double strikePrice = Double.parseDouble(matcher.group(7));
            
            // Handle 2-digit or 4-digit years
            int year = Integer.parseInt(yearStr);
            if (year < 100) {
                year += 2000;
            }
            
            // Parse the date
            String dateStr = String.format("%d %s %d", day, monthStr, year);
            DateTimeFormatter formatter = DateTimeFormatter.ofPattern("d MMM yyyy", Locale.ENGLISH);
            LocalDate expiryDate = LocalDate.parse(dateStr, formatter);
            
            // Validate expiry is not in the past
            if (expiryDate.isBefore(LocalDate.now())) {
                return ParsedOrder.invalid("Expiry date is in the past: " + expiryDate);
            }
            
            // Validate strike price
            if (strikePrice <= 0) {
                return ParsedOrder.invalid("Strike price must be positive: " + strikePrice);
            }
            
            return ParsedOrder.valid(expiryDate, action, symbol, optionType, strikePrice);
            
        } catch (DateTimeParseException e) {
            return ParsedOrder.invalid("Invalid date: " + e.getMessage());
        } catch (NumberFormatException e) {
            return ParsedOrder.invalid("Invalid number format: " + e.getMessage());
        } catch (Exception e) {
            return ParsedOrder.invalid("Parse error: " + e.getMessage());
        }
    }
    
    /**
     * Validates if a string looks like a parseable order without fully parsing it.
     */
    public static boolean looksLikeOrder(String input) {
        if (input == null || input.trim().isEmpty()) {
            return false;
        }
        return ORDER_PATTERN.matcher(input.trim().toLowerCase()).find();
    }
}
